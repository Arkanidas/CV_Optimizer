...
export default function AnalysisConclusion({ conclusion, onContinue }: AnalysisConclusionProps) {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      ...
      <p className="mt-3 text-sm leading-relaxed text-[#EAEAEA]">{conclusion.summary}</p>
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {conclusion.strengths.length > 0 && (
          <div>
            <p className="mb-2 text-sm font-medium uppercase tracking-widest text-emerald-400/95">
              Strengths
            </p>
        ...
        {conclusion.gaps.length > 0 && (
          <div>
            <p className="mb-2 text-sm font-medium uppercase tracking-widest text-rose-400/70">
              Cons
            </p>
            <ul className="flex flex-col gap-1.5">
       ...
       <div className="mt-4 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-4">
         <p className={`text-sm font-medium ${verdictStyle.text}`}>{conclusion.recommendation}</p>
